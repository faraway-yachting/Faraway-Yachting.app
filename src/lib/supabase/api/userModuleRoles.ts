...
export const userModuleRolesApi = {
  ...
  ): Promise<{ user: UserWithModuleRoles | null; error: string | null }> {
    try {
      ...
      if (!response.ok) {
        return { user: null, error: data.error || 'Failed to invite user' };
      }
      ...
      return { user: null, error: 'Failed to invite user' };
    }
  },
