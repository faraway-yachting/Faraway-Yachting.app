...
import { mockDebitNotes } from '@/data/income/mockData';
import { getCompanyById, getActiveCompanies } from '@/data/company/companies';
import type { DebitNoteStatus } from '@/data/income/types';
...
const statusTabs = [
  ...
  { label: 'Draft', value: 'draft' as DebitNoteStatus },
  { label: 'Issued', value: 'issued' as DebitNoteStatus },
  { label: 'Void', value: 'void' as DebitNoteStatus },
];
...
export default function DebitNotesPage() {
  ...
  const [activeTab, setActiveTab] = useState<DebitNoteStatus | 'all' | 'recent'>('recent');
  ...
  // Filter debit notes based on active tab and filters
  const filteredDebitNotes = mockDebitNotes.filter((debitNote) => {
    // Status filter
    if (activeTab !== 'all' && activeTab !== 'recent') {
      if (debitNote.status !== activeTab) return false;
    }

    // Company filter
    if (filters.companyId && debitNote.companyId !== filters.companyId) {
      return false;
    }

    // Currency filter
    if (filters.currency && debitNote.currency !== filters.currency) {
      return false;
    }
    ...
    if (filters.dateFrom) {
      ...
      const debitNoteDate = new Date(debitNote.debitNoteDate);
      if (debitNoteDate < fromDate) return false;
    }

    if (filters.dateTo) {
      const toDate = new Date(filters.dateTo);
      const debitNoteDate = new Date(debitNote.debitNoteDate);
      if (debitNoteDate > toDate) return false;
    }
    ...
    if (searchQuery) {
      ...
      return (
        debitNote.debitNoteNumber.toLowerCase().includes(query) ||
        debitNote.clientName.toLowerCase().includes(query) ||
        (debitNote.reference || '').toLowerCase().includes(query)
      );
    }
  ...
  // Export to CSV
  const handleExport = () => {
    const headers = ['Debit Note No.', 'Company', 'Customer', 'Date', 'Reference', 'Currency', 'Total Amount', 'Status'];
    const rows = filteredDebitNotes.map(dn => [
      dn.debitNoteNumber,
      getCompanyById(dn.companyId)?.name || '-',
      dn.clientName,
      new Date(dn.debitNoteDate).toLocaleDateString('en-US'),
      dn.reference || '-',
      dn.currency,
    ...
    link.setAttribute('download', `debit_notes_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const handleRowClick = (debitNoteId: string) => {
    router.push(`/accounting/manager/income/debit-notes/${debitNoteId}`);
  };

  // Calculate summary statistics
  const stats = {
    total: filteredDebitNotes.length,
    totalAmount: filteredDebitNotes.filter(dn => dn.status === 'issued').reduce((sum, dn) => sum + dn.totalAmount, 0),
  };
          ...
          <div className="flex overflow-x-auto">
            {statusTabs.map((tab) => {
              ...
              if (tab.value === 'all' || tab.value === 'recent') {
                count = mockDebitNotes.length;
              } else {
                count = mockDebitNotes.filter(dn => dn.status === tab.value).length;
              }
        ...
        <div className="p-4 bg-gray-50 border-b border-gray-200">
          <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
            ...
            <div className="w-full sm:w-auto sm:flex-1 sm:max-w-md">
              <input
                type="text"
                placeholder="Search by debit note no., customer, or reference..."
                value={searchQuery}
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              <button
                onClick={() => router.push('/accounting/manager/income/debit-notes/new')}
              ...
              {filteredDebitNotes.length === 0 ? (
                ...
                filteredDebitNotes.map((debitNote) => {
                  return (
                    <tr
                      key={debitNote.id}
                      onClick={() => handleRowClick(debitNote.id)}
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-gray-600">
                            {getCompanyById(debitNote.companyId)?.name || '-'}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">{debitNote.clientName}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-600">
                          {new Date(debitNote.debitNoteDate).toLocaleDateString('en-US', {
                            year: 'numeric',
                            month: 'short',
                            day: 'numeric',
                          })}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        <div className="text-sm font-medium text-gray-900">
                          {debitNote.totalAmount.toLocaleString('en-US', {
          ...
          <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
            <div className="flex justify-between items-center text-sm">
              <div className="text-gray-600">
                Showing <span className="font-semibold text-gray-900">{filteredDebitNotes.length}</span> debit note{filteredDebitNotes.length !== 1 ? 's' : ''}
              </div>
