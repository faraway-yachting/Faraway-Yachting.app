...
export function PLReportTable({
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden mb-6">
      {/* Header */}
      <div className={`${headerColor} px-3 md:px-4 py-3 border-b border-gray-200`}>
        <div className="flex items-center justify-between">
          <h3 className={`text-base md:text-lg font-semibold ${headerTextColor}`}>{title}</h3>
          <span className={`text-base md:text-lg font-bold ${headerTextColor}`}>
      ...
      <table className="w-full">
        <thead>
          <tr className="bg-gray-50 border-b border-gray-200">
            ...
            <th className="text-right px-3 md:px-4 py-2 text-xs font-medium text-gray-500 uppercase tracking-wider">
        ...
        <tbody className="divide-y divide-gray-200">
          {categories.map((category) => (
            <Fragment key={category.code}>
              ...
              >
                <td colSpan={isMobile ? 1 : (showInTHB ? 5 : 6)} className="px-3 md:px-4 py-2">
              ...
              {expandedCategories.has(category.code) &&
                category.items.map((item, index) => (
                  <tr key={`${item.id}-${item.documentNumber}-${index}`} className="hover:bg-gray-50">
                    <td className="px-3 md:px-4 py-2 text-sm text-gray-600">
                    ...
                    <td className="px-3 md:px-4 py-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-blue-600 hover:underline cursor-pointer truncate">
                          {item.documentNumber}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-3 md:px-4 py-2 text-sm font-medium text-gray-900 text-right">
                      {showInTHB
          ...
          {categories.length === 0 && (
            <tr>
              <td
                colSpan={isMobile ? 3 : (showInTHB ? 6 : 7)}
