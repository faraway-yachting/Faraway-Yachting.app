...
import type { CreditNoteReason, LineItem, PricingType } from '@/data/income/types';
...
export default function CreditNotePrintView({
  ...
  // Get PDF field settings for credit notes (use invoice settings as base)
  const fieldSettings = getDocumentFieldSettings('invoice');

  // Filter valid line items
  const validLineItems = useMemo(() =>
    creditNote.lineItems.filter((item) => item.description.trim() !== '' || item.unitPrice > 0),
  ...
  // Set document title for PDF filename: "Company CreditNoteNumber"
  useEffect(() => {
    if (isOpen) {
      const originalTitle = document.title;
      const companyName = company?.name || 'CreditNote';
  ...
  }, [isOpen, company?.name, creditNote.creditNoteNumber]);
  ...
  const hasWht = creditNote.lineItems.some(
      ...
      <div className="grid grid-cols-2 gap-6 mt-4">
        ...
        <div className="text-right">
          <table className="ml-auto text-xs">
            <tbody>
              <tr>
                <td className="py-0.5 text-gray-500 pr-3">Credit Note No:</td>
              ...
              <tr>
                ...
                <td className="py-0.5 text-gray-900">{formatDate(creditNote.creditNoteDate)}</td>
              </tr>
              {creditNote.reference && (
                <tr>
                  <td className="py-0.5 text-gray-500 pr-3">Reference:</td>
                  <td className="py-0.5 text-gray-900">{creditNote.reference}</td>
                </tr>
              )}
              <tr>
                <td className="py-0.5 text-gray-500 pr-3">Reason:</td>
                <td className="py-0.5 text-gray-900">{reasonLabels[creditNote.reason]}</td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  const renderFooter = () => (
    <div className="print-footer">
              ...
              {fieldSettings.showSubtotal && (
                <tr>
                  ...
                  <td className="py-1 text-right text-gray-900">
                    {formatCurrency(creditNote.subtotal, creditNote.currency as any)}
                  </td>
                </tr>
              )}
              {fieldSettings.showVatAmount && creditNote.pricingType !== 'no_vat' && (
                <tr>
                  <td className="py-1 text-gray-600">
                    VAT ({creditNote.pricingType === 'include_vat' ? 'Included' : 'Added'}):
                  </td>
                  <td className="py-1 text-right text-gray-900">
                    {formatCurrency(creditNote.taxAmount, creditNote.currency as any)}
                  </td>
                </tr>
              )}
              {fieldSettings.showWhtAmount && creditNote.whtAmount > 0 && (
                <tr>
                  <td className="py-1 text-gray-600">Withholding Tax:</td>
                  <td className="py-1 text-right text-gray-900">
                    -{formatCurrency(creditNote.whtAmount, creditNote.currency as any)}
                  </td>
                </tr>
              )}
              <tr className="border-t-2 border-gray-800">
                <td className="py-2 font-bold text-gray-900 text-sm">Total Credit:</td>
                <td className="py-2 text-right font-bold text-gray-900 text-sm">
                  {formatCurrency(creditNote.totalAmount, creditNote.currency as any)}
                </td>
              </tr>
            </tbody>
          </table>
      ...
      {/* Notes */}
      {fieldSettings.showTermsAndConditions && creditNote.notes && (
        <div className="mb-3">
          <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">
            Notes
          </h3>
      ...
      {fieldSettings.showCreatedBySignature && (
        <div className="mt-3">
          <div className="w-40">
            ...
            <p className="text-xs text-gray-500">{formatDate(creditNote.creditNoteDate)}</p>
          </div>
        </div>
      )}
    </div>
      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="bg-gray-100">
            ...
            {fieldSettings.showVatColumn && creditNote.pricingType !== 'no_vat' && (
            ...
            pageItems.map((item, itemIndex) => (
              <tr key={item.id} className="border-b border-gray-200">
                ...
                <td className="py-2 px-3 text-right text-gray-900">
                  {formatCurrency(item.unitPrice, creditNote.currency as any)}
                </td>
                {fieldSettings.showVatColumn && creditNote.pricingType !== 'no_vat' && (
                  ...
                  <td className="py-2 px-3 text-center text-gray-600">
                    {item.whtRate !== 0 && item.whtRate !== 'custom'
                      ...
                      : item.whtRate === 'custom' && item.customWhtAmount
                        ? formatCurrency(item.customWhtAmount, creditNote.currency as any)
                        : '-'}
                  </td>
                )}
                <td className="py-2 px-3 text-right font-medium text-gray-900">
                  {formatCurrency(item.amount, creditNote.currency as any)}
                </td>
              </tr>
            ))
          )}
