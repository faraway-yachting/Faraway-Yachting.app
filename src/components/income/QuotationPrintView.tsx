...
export default function QuotationPrintView({
  ...
  // Get PDF field settings for quotations
  const fieldSettings = getDocumentFieldSettings('quotation');

  // Filter valid line items
  const validLineItems = useMemo(() =>
    quotation.lineItems.filter((item) => item.description.trim() !== '' || item.unitPrice > 0),
    [quotation.lineItems]
  );

  // Split line items into pages with smart pagination
  // Structure: content pages have items, last page always has footer
  // If items fit on first page with footer, single page
  // Otherwise: first page items -> middle pages items -> last page with remaining items + footer
  const { contentPages, needsSeparateFooterPage } = useMemo(() => {
    ...
    // If there are remaining items, they go on a page with footer
    // If no remaining items, footer goes on its own page
    if (items.length > 0) {
      pages.push(items);
  ...
  // Set document title for PDF filename: "Company QuotationNumber"
  useEffect(() => {
    if (isOpen) {
      const originalTitle = document.title;
      const companyName = company?.name || 'Quotation';
      const docNumber = quotation.quotationNumber || 'Draft';
      document.title = `${companyName} ${docNumber}`;
      return () => {
        document.title = originalTitle;
      };
    }
  }, [isOpen, company?.name, quotation.quotationNumber]);
    ...
    <div className="print-header">
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="text-right">
          <h2 className="text-2xl font-bold tracking-wider text-gray-800">QUOTATION</h2>
      ...
      <div className="grid grid-cols-2 gap-6 mt-4">
        ...
        <div className="text-right">
          <table className="ml-auto text-xs">
            <tbody>
              <tr>
                <td className="py-0.5 text-gray-500 pr-3">Quotation No:</td>
              ...
              <tr>
                ...
                <td className="py-0.5 text-gray-900">{formatDate(quotation.dateCreated)}</td>
              </tr>
              {fieldSettings.showValidUntil && (
                <tr>
                  <td className="py-0.5 text-gray-500 pr-3">Valid Until:</td>
                  <td className="py-0.5 text-gray-900">{formatDate(quotation.validUntil)}</td>
                </tr>
              )}
            </tbody>
          </table>
  ...
  const renderFooter = () => (
    <div className="print-footer">
        ...
        <div className="w-64">
          <table className="w-full text-xs">
            <tbody>
              ...
              {fieldSettings.showVatAmount && quotation.pricingType !== 'no_vat' && (
                <tr>
                  <td className="py-1 text-gray-600">
                    VAT ({quotation.pricingType === 'include_vat' ? 'Included' : 'Added'}):
                  </td>
                  <td className="py-1 text-right text-gray-900">
              ...
              <tr className="border-t-2 border-gray-800">
                ...
                <td className="py-2 text-right font-bold text-gray-900 text-sm">
                  {formatCurrency(quotation.totalAmount, quotation.currency as any)}
                </td>
              </tr>
              {fieldSettings.showWhtAmount && quotation.whtAmount > 0 && (
                <>
                  <tr>
                    ...
                    <td className="py-1 text-right text-gray-900">
                      -{formatCurrency(quotation.whtAmount, quotation.currency as any)}
                    </td>
                  </tr>
                  {fieldSettings.showNetAmountToPay && (
                    <tr className="border-t border-gray-300">
                      ...
                      <td className="py-1 text-right font-semibold text-gray-900">
                        {formatCurrency(quotation.totalAmount - quotation.whtAmount, quotation.currency as any)}
                      </td>
                    </tr>
                  )}
                </>
      ...
      {/* Terms & Conditions */}
      {fieldSettings.showTermsAndConditions && quotation.termsAndConditions && (
        <div className="mb-3">
          <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">
            Terms & Conditions
          </h3>
      ...
      {fieldSettings.showCreatedBySignature && (
        <div className="mt-4">
          <div className="w-40">
            ...
            <p className="text-xs text-gray-500">{formatDate(quotation.dateCreated)}</p>
          </div>
        </div>
      )}
    </div>
      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="bg-gray-100">
            ...
            {fieldSettings.showVatColumn && quotation.pricingType !== 'no_vat' && (
                  ...
                  <td className="py-2 px-3 text-center text-gray-600">
                    {item.whtRate !== 0 && item.whtRate !== 'custom'
                      ...
                      : item.whtRate === 'custom' && item.customWhtAmount
                        ? formatCurrency(item.customWhtAmount, quotation.currency as any)
                        : '-'}
                  </td>
                )}
