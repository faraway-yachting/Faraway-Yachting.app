...
export default function DebitNoteForm({ debitNote, receiptId, onCancel }: DebitNoteFormProps) {
  ...
  // Form state - initialized from debit note, receipt, or defaults
  const [companyId, setCompanyId] = useState(
    debitNote?.companyId || sourceReceipt?.companyId || ''
  );
  const [clientId, setClientId] = useState(
    debitNote?.clientId || sourceReceipt?.clientId || ''
  );
  const [clientName, setClientName] = useState(
    debitNote?.clientName || sourceReceipt?.clientName || ''
  );
  const [debitNoteDate, setDebitNoteDate] = useState(
    debitNote?.debitNoteDate || getTodayISO()
  );
  const [currency, setCurrency] = useState<Currency>(
    debitNote?.currency || sourceReceipt?.currency || 'USD'
  );

  // Debit note number - generate if new, use existing if editing
  const initialDebitNoteNumber = debitNote?.debitNoteNumber || '';
  const [debitNoteNumber, setDebitNoteNumber] = useState(initialDebitNoteNumber);

  // Reference - set to receipt number when creating from receipt
  const [reference, setReference] = useState(
    debitNote?.reference || sourceReceipt?.receiptNumber || ''
  );

  // Pricing type
  const [pricingType, setPricingType] = useState<PricingType>(
    debitNote?.pricingType || sourceReceipt?.pricingType || 'exclude_vat'
  );

  // Line items - for debit notes from receipt, start with empty line items
  // (user should specify additional charges/late fees)
  ...
  const [notes, setNotes] = useState(
    debitNote?.notes || (isEditing ? '' : getDefaultTermsAndConditions('invoice'))
  );
  ...
  useEffect(() => {
    if (!isEditing && companyId) {
      const companyDebitNotes = getDebitNotesByCompany(companyId);
      const newNumber = generateDebitNoteNumber(companyId, companyDebitNotes.length);
  ...
  const handleSave = async (status: 'draft' | 'issued') => {
    ...
    try {
      const debitNoteData: Partial<DebitNote> = {
        companyId,
        clientId,
        clientName,
      ...
      // Create or update debit note
      if (isEditing && debitNote) {
        const updated = updateDebitNote(debitNote.id, debitNoteData);
        console.log('Updated debit note:', updated);
        if (updated) {
          setCurrentStatus(status);
          setIsEditMode(false); // Switch to view mode after save
        }
      } else {
        const newDebitNote = createDebitNote(debitNoteData);
        console.log('Created debit note:', newDebitNote);
        // Navigate to edit page for the newly created debit note
        if (newDebitNote) {
          router.push(`/accounting/manager/income/debit-notes/${newDebitNote.id}`);
        }
      }
    } catch (error) {
  ...
  const handleVoid = async () => {
    ...
    setIsSaving(true);
    try {
      const updated = updateDebitNote(debitNote.id, {
        status: 'void',
