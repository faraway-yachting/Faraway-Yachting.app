...
import type { DebitNoteReason, LineItem, PricingType } from '@/data/income/types';
...
export default function DebitNotePrintView({
  debitNote,
  company,
  client,
  clientName,
  ...
  isOpen,
  onClose,
}: DebitNotePrintViewProps) {
  const [mounted, setMounted] = useState(false);

  // Get PDF field settings for debit notes (use invoice settings as base)
  const fieldSettings = getDocumentFieldSettings('invoice');

  // Filter valid line items
  const validLineItems = useMemo(() =>
    debitNote.lineItems.filter((item) => item.description.trim() !== '' || item.unitPrice > 0),
    [debitNote.lineItems]
  );
  ...
  // Set document title for PDF filename: "Company DebitNoteNumber"
  useEffect(() => {
    if (isOpen) {
      const originalTitle = document.title;
      const companyName = company?.name || 'DebitNote';
      const docNumber = debitNote.debitNoteNumber || 'Draft';
      document.title = `${companyName} ${docNumber}`;
      return () => {
        document.title = originalTitle;
      };
    }
  }, [isOpen, company?.name, debitNote.debitNoteNumber]);
  ...
  const hasWht = debitNote.lineItems.some(
      ...
      <div className="grid grid-cols-2 gap-6 mt-4">
        ...
        <div className="text-right">
          <table className="ml-auto text-xs">
            <tbody>
              <tr>
                <td className="py-0.5 text-gray-500 pr-3">Debit Note No:</td>
                <td className="py-0.5 font-semibold text-gray-900">
                  {debitNote.debitNoteNumber || 'Draft'}
                </td>
              </tr>
              <tr>
                <td className="py-0.5 text-gray-500 pr-3">Date:</td>
                <td className="py-0.5 text-gray-900">{formatDate(debitNote.debitNoteDate)}</td>
              </tr>
              {debitNote.reference && (
                <tr>
                  <td className="py-0.5 text-gray-500 pr-3">Reference:</td>
                  <td className="py-0.5 text-gray-900">{debitNote.reference}</td>
                </tr>
              )}
              <tr>
                <td className="py-0.5 text-gray-500 pr-3">Reason:</td>
                <td className="py-0.5 text-gray-900">{reasonLabels[debitNote.reason]}</td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  const renderFooter = () => (
    <div className="print-footer">
              ...
              {fieldSettings.showSubtotal && (
                <tr>
                  ...
                  <td className="py-1 text-right text-gray-900">
                    {formatCurrency(debitNote.subtotal, debitNote.currency as any)}
                  </td>
                </tr>
              )}
              {fieldSettings.showVatAmount && debitNote.pricingType !== 'no_vat' && (
                <tr>
                  <td className="py-1 text-gray-600">
                    VAT ({debitNote.pricingType === 'include_vat' ? 'Included' : 'Added'}):
                  </td>
                  <td className="py-1 text-right text-gray-900">
                    {formatCurrency(debitNote.taxAmount, debitNote.currency as any)}
                  </td>
                </tr>
              )}
              {fieldSettings.showWhtAmount && debitNote.whtAmount > 0 && (
                <tr>
                  <td className="py-1 text-gray-600">Withholding Tax:</td>
                  <td className="py-1 text-right text-gray-900">
                    -{formatCurrency(debitNote.whtAmount, debitNote.currency as any)}
                  </td>
                </tr>
              )}
              <tr className="border-t-2 border-gray-800">
                <td className="py-2 font-bold text-gray-900 text-sm">Total Due:</td>
                <td className="py-2 text-right font-bold text-gray-900 text-sm">
                  {formatCurrency(debitNote.totalAmount, debitNote.currency as any)}
                </td>
              </tr>
            </tbody>
          </table>
      ...
      {/* Notes */}
      {fieldSettings.showTermsAndConditions && debitNote.notes && (
        <div className="mb-3">
          <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">
            Notes
          </h3>
      ...
      {fieldSettings.showCreatedBySignature && (
        <div className="mt-3">
          <div className="w-40">
            ...
            <p className="text-xs text-gray-500">{formatDate(debitNote.debitNoteDate)}</p>
          </div>
        </div>
      )}
    </div>
      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="bg-gray-100">
            ...
            {fieldSettings.showVatColumn && debitNote.pricingType !== 'no_vat' && (
            ...
            pageItems.map((item, itemIndex) => (
              <tr key={item.id} className="border-b border-gray-200">
                ...
                <td className="py-2 px-3 text-right text-gray-900">
                  {formatCurrency(item.unitPrice, debitNote.currency as any)}
                </td>
                {fieldSettings.showVatColumn && debitNote.pricingType !== 'no_vat' && (
                  ...
                  <td className="py-2 px-3 text-center text-gray-600">
                    {item.whtRate !== 0 && item.whtRate !== 'custom'
                      ...
                      : item.whtRate === 'custom' && item.customWhtAmount
                        ? formatCurrency(item.customWhtAmount, debitNote.currency as any)
                        : '-'}
                  </td>
                )}
                <td className="py-2 px-3 text-right font-medium text-gray-900">
                  {formatCurrency(item.amount, debitNote.currency as any)}
                </td>
              </tr>
            ))
          )}
